...
db.serialize(() => {
    ...
    // Create plans table
    db.run("CREATE TABLE IF NOT EXISTS plans (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, description TEXT, price REAL, duration INTEGER, daily_limit INTEGER, status TEXT DEFAULT 'active', created_at TEXT)", (err) => {
        if (!err) {
            // Check if plans exist, if not, seed default plans
            db.get("SELECT COUNT(*) as count FROM plans", (err, row) => {
                if (!err && row && row.count === 0) {
                    console.log("Seeding default subscription plans...");
                    const stmt = db.prepare("INSERT INTO plans (name, description, price, duration, daily_limit, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    const now = new Date().toISOString();
                    
                    const defaultPlans = [
                        ['Test Pro', 'Testing', 50, 15, 4, 'active', now],
                        ['Standard', 'Standard', 100, 30, 20, 'active', now],
                        ['Platinum', 'Platinum for users', 10, 10, 10, 'active', now],
                        ['Silver', 'Most Valuable Plan', 10, 5, 10, 'active', now]
                    ];
...
app.post('/api/admin/users/:id/reset-ads', (req, res) => {
    ...
    // Deleting Ad View records from deposits table effectively resets them
    // because the system checks this table to see if an ad is "completed"
    db.run("DELETE FROM deposits WHERE user_id = ? AND gateway = 'Ad View'", [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: 'DB Error' });
